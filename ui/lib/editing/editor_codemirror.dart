  ...
  Editor createFromElement(html.Element element, {Map options}) {
    if (options == null) {
      options = {
        ...
        'viewportMargin': 100,
//        'lineNumbers': true,
        'gutters': ['breakpoints', 'line-infos'],
...
class _CodeMirrorEditor extends Editor {
  ...
  void clearBreakpoints() {
    ...
    html.querySelectorAll('.breakpoint-line-highlight').forEach(
        (e) => e.classes.remove('breakpoint-line-highlight'));
  }

  void setBreakpoint(int lineNumber) {
    cm.setGutterMarker(lineNumber, 'breakpoints', makeMarker());
    cm.addLineClass(lineNumber, 'background', 'breakpoint-line-highlight');
  }

  html.Element makeMarker() {
    html.DivElement marker = html.document.createElement('div');
    marker.classes.add('breakpoint');
    marker.setInnerHtml(
        '''<svg width="10" height="10" viewBox="0 0 20 20">
             <path d="M 0 6 h 10 v -6 L 20 10 L 10 20 v -6 h -10 Z"></path>
           </svg>''', validator: new html.NodeValidatorBuilder()..allowSvg());
    return marker;
  }

  void clearLineInfos() {
    cm.clearGutter('line-infos');
  }

  void setLineInfo(int lineNumber, String info) {
    html.Element infoElement = makeInfo(info);
    cm.setGutterMarker(lineNumber, 'line-infos', infoElement);
    html.DivElement parent = infoElement.parent.parent;
    parent.onMouseDown.listen((e) => e.stopPropagation());
    parent.onMouseMove.listen((e) => e.stopPropagation());
    parent.onMouseUp.listen((e) => e.stopPropagation());
    parent.onClick.listen((e) => e.stopPropagation());
    parent.onContextMenu.listen((e) => e.stopPropagation());
  }

  html.Element makeInfo(String info) {
    var marker = html.document.createElement('div');
    marker.text = info;
    marker.classes.add('line-info');
    return marker;
  }
}
